...
function Save3(code){
	...
	document.cookie = "rox=" + tmp + "; expires=Fri, 31-Dec-2030 23:59:59; ";
	setAlertLayer("The data has been saved to  Slot "+num+".",1);
}

function codeCopy(){
	tmp2 = "http://xellie.com/RO/skill/"+job+".html?"+Save("scode");
	setAlertLayer("<br><br><br><br>Copy and paste to share your build<br><br><br>"
...
function msovr(posX,posY,skillid,flg){
	...
	document.getElementById("skill").style.border = "solid 1px #000000";
		document.getElementById("skill").style.borderRadius = "20px";
	document.getElementById("skill").innerHTML = "<div id='skillbox' style='padding:10px; line-height:14px;  white-space: normal;'>" + tmpTxt + "</div>";
...
function setOption2Layer(){
	tmpTxt = "<form name='Option2' onSubmit='return false;'>"
		+ "<textarea name='sdata' id='sdata' cols=70 rows=10 wrap=hard readonly style='background-color: transparent;'>"
		+ "</textarea><br>"
	//	+ "<input class='con' type=checkbox name='space' onClick=\"setArray('"+job+"')\"> Space"
	+ "<div align='left' style='margin-left: 40px;'>"
     + "<input class='con' type=checkbox name='line' onClick=\"setArray('"+job+"')\"> List </br>" 
		+ "<input class='con' type=checkbox name='type' onClick=\"setArray('"+job+"')\">Shortened Skill List </br>"
		+ "<input class='con' type=checkbox name='comma' onClick=\"setArray('"+job+"')\">Comma </br> </div>" 
		+ "<input class='btn' type=button value='Copy to Clipboard' onClick=\"copyText()\">"
		+ "</form>";
...
function setAlertLayer(txt,close){
	...
	document.getElementById("option2").style.height = '300px';
	document.getElementById("option2").style.width = '600px';
	document.getElementById("option2").style.top = '100px';
	document.getElementById("option2").style.left = '300px';
//	alert(navigator.userAgent);
	...
	document.getElementById("option2").style.border = ' 1px solid #333';
	document.getElementById("option2").style.borderRadius="25px"
	document.getElementById("option2").style.padding = '0px';
		document.getElementById("option2").style.backgroundImage = 'url(img/15.png)';
			document.getElementById("option2").style.backgroundPosition = "Bottom Right";
			document.getElementById("option2").style.backgroundRepeat = "no-repeat";
			document.getElementById("option2").style.boxShadow = "10px 20px 30px #000";
...
function copyText() {
  /* Get the text field */
  var copyText = document.getElementById("sdata");
